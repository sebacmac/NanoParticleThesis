...
\begin{tikzpicture}
	...
	\node[anchor=south west,inner sep=0] at (0,0) {\includegraphics[width=10cm]{out}};
	
	\fill[fill=black] (2.1 ,4.9) circle (0.1) node[left] {$A$};
	...
	\fill[fill=black] (7.8 ,4.9) circle (0.1) node[right] {$B$};
	\draw[dashed, thick] (2.1,4.9) -- (7.8,4.9);
	\node[label=below:$s$] at (4.5,5.1) {};
	\node[anchor=west] (w) at (5,5.5) {windows};
	\draw[] (1) -- (w);
