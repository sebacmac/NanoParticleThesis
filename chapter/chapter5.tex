...
\begin{table}[h!t]
	...
	\begin{tabular}{lcccc}
		\toprule
		\,		& striped ($1$:$1$)		& random ($1$:$1$)		& random ($1$:$2$)	& \multirow{2}{*}{atomistic$^b$}	\\
		\,		& \acs{PW}$+$\acs{PME}$^a$ & \acs{PW}$+$\acs{PME}$^a$ & \acs{PW}$+$\acs{PME}$^a$ &	\\ \toprule
...
\begin{figure}[p]
	...
	\end{adjustwidth}
	\caption{Density map for the striped (\acs{MUS}:\acs{OT} $1$:$1$) \acs{NP} in the hydrophobic state of an unbiased run. (a), (c) and (e) refer to the top leaflet; (b), (d) and (f) refer to the bottom leaflet (the entrance leaflet).}%
\end{figure}
...
\begin{figure}[p]
	...
	\end{adjustwidth}
	\caption{Density map for the random (\acs{MUS}:\acs{OT} $1$:$1$) \acs{NP} in the hydrophobic state for an unbiased run. (a), (c) and (e) refer to the top leaflet (entrance leaflet); (b), (d) and (f) refer to the bottom leaflet.}%
\end{figure}

\begin{figure}[p]
	\centering
	\begin{adjustwidth}{-0.5cm}{-1.5cm}
		\subfloat[Top leaflet]{%
		\includegraphics[width=0.5\textwidth]{./img/density_RDF/patched/lipidTopRDF}
		}\,%
		\subfloat[Bottom leaflet]{%
		\includegraphics[width=0.5\textwidth]{./img/density_RDF/patched/lipidBottomRDF}
		}%
		\caption{\acs{RDF} centered to the \acs{NP} \acs{COM} of the lipid tail (green) and the lipid head (violet) for the striped (\acs{MUS}:\acs{OT} $1$:$1$) \acs{NP} in the hydrophobic state of an unbiased run. (a) refer to the top leaflet. (b) refer to the bottom leaflet (the entrance leaflet).}%
		...
		\caption{\acs{RDF} centered to the \acs{NP} \acs{COM} of the lipid tail (green) and the lipid head (violet) for the random (\acs{MUS}:\acs{OT} $1$:$1$) \acs{NP} in the hydrophobic state of an unbiased run. (a) refer to the top leaflet (the entrance leaflet). (b) refer to the bottom leaflet.}%
	\end{adjustwidth}
\end{figure}
